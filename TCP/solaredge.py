...
def calcShift (number):
    places = 65536 - number
    scale = '0.'+str(1).zfill(places) # pad out our scale according to places shift value
    return float(scale) #return out new number as a float
...
def readAll():
    ...
    global client
    base = 0x0018
    rq = client.read_holding_registers(base, 45) #start at 69 to get correct readings  
    if debug > 0:
